...
function EngravingField({activity=0}:{activity?:number}){
  ...
  useEffect(()=>{if(window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;const canvas=ref.current;if(!canvas)return;const context=canvas.getContext('2d');if(!context)return;const ctx=context;let w=0,h=0,dpr=1,raf=0;const pointer={x:.5,y:.5,tx:.5,ty:.5};
    ...
    const draw=(ms:number)=>{const t=ms*.00012;pointer.x+=(pointer.tx-pointer.x)*.025;pointer.y+=(pointer.ty-pointer.y)*.025;ctx.clearRect(0,0,w,h);const cx=w*(.58+(pointer.x-.5)*.035),cy=h*(.46+(pointer.y-.5)*.035);const unit=Math.min(w,h);ctx.save();ctx.translate(cx,cy);ctx.rotate(-.11+Math.sin(t*.8)*.01);
      const ringCount=w<700?18:32,samples=w<700?88:140;for(let ring=0;ring<ringCount;ring++){const r=unit*(.14+ring*.0138),amp=unit*(.017+((ring%7)*.0018)),phase=t*(.65+(ring%5)*.06)+ring*.31;ctx.beginPath();for(let i=0;i<=samples;i++){const a=i/samples*Math.PI*2,mod=Math.sin(a*6+phase)*amp+Math.sin(a*11-phase*.7)*amp*.34,x=Math.cos(a)*(r+mod),y=Math.sin(a)*(r*.42+mod*.22);if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)}ctx.strokeStyle=`rgba(201,255,112,${.012+ring*.0005})`;ctx.lineWidth=.7;ctx.stroke()}
      ctx.restore();
      const lanes=8+Math.min(10,activityRef.current);for(let i=0;i<lanes;i++){const y=(h/(lanes+1))*(i+1),offset=((ms*.018+i*173)%(w+340))-170;ctx.beginPath();ctx.moveTo(offset-120,y);ctx.lineTo(offset+120,y);const g=ctx.createLinearGradient(offset-120,y,offset+120,y);g.addColorStop(0,'rgba(201,255,112,0)');g.addColorStop(.5,'rgba(201,255,112,.1)');g.addColorStop(1,'rgba(201,255,112,0)');ctx.strokeStyle=g;ctx.lineWidth=1;ctx.stroke()}
...
function Certificate({state,status}:{state?:StrategyState;status:string}){
  ...
  return <div className="certificate-stage">
    <div className="certificate-shadow"/><div className="certificate-underlay underlay-a"/><div className="certificate-underlay underlay-b"/>
    <article className="certificate">
      <div className="certificate-guilloche g1"/><div className="certificate-guilloche g2"/>
      <div className="certificate-watermark-mark"><Seal/></div>
      <header className="certificate-top"><span>COMMON STRATEGY / GENESIS TREASURY INSTRUMENT</span><b>{state?`BOARD ${state.boardVersion}`:'ISSUE 000'}</b></header>
      <div className="certificate-main">
        <div className="certificate-identity-block"><Seal className="certificate-seal"/><div><small>COMMONS TREASURY COMPANY</small><h1>COMMON <em>STRATEGY</em></h1><p>PUBLIC ACCUMULATION VEHICLE</p></div></div>
        <div className="certificate-allocation"><span>VOUCHER ALLOCATION</span><strong>100%</strong><small>FOUNDER / TEAM 0.00%</small></div>
      </div>
      <div className="certificate-pledge"><span>THE ENTIRE FINAL COMMONS ALLOCATION IS PLEDGED TO POSITIVE VOUCHERS, PRO RATA BY POINTS CONTRIBUTED.</span></div>
      <div className="certificate-score"><span>COMMONS POINTS HELD</span><strong>{state?<AnimatedNumber value={state.totalPoints}/>:<>000,000</>}</strong><em>{state?`LIVE · ${state.voucherCount} OWNER${state.voucherCount===1?'':'S'}`:'PRE-LIVE · UNISSUED'}</em></div>
      <div className="certificate-metrics"><div><span>RANK</span><b>{state?`#${nf.format(state.rank)}`:'—'}</b></div><div><span>VOUCHERS</span><b>{state?nf.format(state.voucherCount):'0'}</b></div><div><span>VOUCH POINTS</span><b>{state?compact(state.positiveVouchPoints):'0'}</b></div><div><span>LAST ENTRY</span><b>{recent?`@${recent.handle} ${recent.points>0?'+':''}${compact(recent.points)}`:'NONE'}</b></div></div>
      <footer className="certificate-bottom"><div><span>STATUS</span><b>{status}</b></div><div><span>SOURCE</span><b>COMMONS GENESIS LEDGER</b></div><div className="certificate-serial">CS / {state?String(state.boardVersion).padStart(6,'0'):'000000'} / GENESIS</div></footer>
    </article>
  </div>;
}
...
export function CommonStrategy({initial,error:initialError}:{initial?:StrategyState;error?:string}){
  ...
  return <main className="treasury-app">
    ...
    <header className="site-header"><a className="identity" href="#top" aria-label="Common Strategy, back to top"><Seal/><span><b>COMMON</b><em>STRATEGY</em></span></a><nav aria-label="Primary navigation"><a href="#ownership">OWNERSHIP</a><a href="#calculator">CALCULATOR</a><a href="#ledger">LEDGER</a><a href="#pledge">PLEDGE</a></nav><div className={`header-status ${state?'live':'waiting'}`}><i/><span>{status}</span></div></header>
    <MarketTape state={state}/>
    <section className="hero-floor" id="top" aria-labelledby="main-content">
      <aside className="hero-brief"><span className="section-code">CS / GENESIS / 01</span><p className="hero-kicker">A PUBLIC ACCUMULATION VEHICLE FOR THE COMMONS EXPERIMENT.</p><h2 id="main-content">THE AIRDROP<br/>BELONGS TO<br/><em>THE VOUCHERS.</em></h2><p className="hero-copy">Common Strategy pledges 100% of any final Commons allocation it receives to the accounts that positively vouched it, weighted by the points each vouch contributed.</p><a className="vouch-button" href={vouchHref} target="_blank" rel="noreferrer"><span>VOUCH @COMMONSTRAT</span><b aria-hidden="true">↗</b></a><div className="brief-foot"><span>FOUNDER SHARE <b>0.00%</b></span><span>VOUCHER SHARE <b>100.00%</b></span></div></aside>
      <Certificate state={state} status={status}/>
      <aside className="hero-flow"><div className="flow-title"><span>LIVE CAPITAL FLOW</span><i/></div><div className="flow-stream">{state?.tape.slice(0,5).map((x,i)=><a href={x.tweetUrl||`https://x.com/${x.handle}`} target="_blank" rel="noreferrer" key={x.id} className={x.kind==='slash'?'negative':''}><small>{String(i+1).padStart(2,'0')}</small><strong>@{x.handle}</strong><b>{x.points>0?'+':''}{compact(x.points)}</b></a>)??<></>}{!state?.tape.length&&<div className="flow-empty"><Seal/><span>WAITING FOR THE FIRST VOUCH TO ENTER THE TAPE.</span></div>}</div><div className="flow-metrics"><div><span>POINTS</span><b>{state?compact(state.totalPoints):'—'}</b>{motion.score!==0&&<em>{motion.score>0?'+':''}{compact(motion.score)}</em>}</div><div><span>RANK</span><b>{state?`#${state.rank}`:'—'}</b>{motion.rank!==0&&<em>{motion.rank>0?'↑':'↓'} {Math.abs(motion.rank)}</em>}</div><div><span>OWNERS</span><b>{state?state.voucherCount:'0'}</b>{motion.owners>0&&<em>+{motion.owners}</em>}</div></div></aside>
    </section>
